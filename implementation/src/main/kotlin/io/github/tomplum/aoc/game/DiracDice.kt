...
class DiracDice(data: List<String>) {
    ...
    fun playWithQuantumDice(): Long = quantumDie.forEachRollPossibility { value ->
        roll(playerOneStart, playerTwoStart, 0, 0, value, true)
    }.let { (playerOneWins, playerTwoWins) -> maxOf(playerOneWins, playerTwoWins) }
    ...
    fun roll(p1: Int, p2: Int, p1Score: Int, p2Score: Int, roll: Int, p1Turn: Boolean): Pair<Long, Long> {
        var playerOnePos = p1
        var playerTwoPos = p2
        ...
        if (playerOneScore >= 21 || playerTwoScore >= 21) {
            return if (playerOneScore >= 21) Pair(1, 0) else Pair(0, 1)
        } else {
            val otherWins = quantumDie.forEachRollPossibility { value ->
                roll(playerOnePos, playerTwoPos, playerOneScore, playerTwoScore, value, !p1Turn)
            }
            wins = Pair(wins.first + otherWins.first, wins.second + otherWins.second)
        }
